...
Le programme \cmd{openocd} nécessite un accès au périphérique USB correspondant à la carte. Si la commande renvoie une erreur du type \texttt{Error: libusb\_open() failed}, il faut ajouter une règle à \cmd{udev} pour donner au développeur les droits d'accéder à la carte, en créant par exemple le fichier \texttt{/etc/udev/rules.d/99-STM32.rules} :
